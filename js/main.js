(function () {
  ...
  function renderEvents(eventsData, settings) {
    ...
    grid.innerHTML = events.map((ev) => {
      ...
      const detailUrl = eventDetailUrl(ev, eventsData.events);
  ...
  function eventDetailUrl(ev, allEvents) {
    if (ev.slug) return `/etkinlik.html?slug=${encodeURIComponent(ev.slug)}`;
    return `/etkinlik.html?i=${allEvents.indexOf(ev)}`;
  }

  function findEvent(eventsData) {
    const params = new URLSearchParams(location.search);
    const slug = params.get("slug");
    const i = params.get("i");
    const events = eventsData.events || [];
    if (slug) return events.find((e) => e.slug === slug) || null;
    if (i !== null && events[Number(i)]) return events[Number(i)];
    return null;
  }

  function renderEventDetail(eventsData, settings) {
    const wrap = $("#event-detail");
    if (!wrap) return;

    const ev = findEvent(eventsData);
    if (!ev) {
      wrap.innerHTML = `<div class="empty-state">Etkinlik bulunamadı. <a href="/etkinlikler.html">Tüm etkinliklere dönün</a>.</div>`;
      return;
    }

    document.title = `${ev.title} | BÜKÜ ART`;
    ...
    wrap.innerHTML = `
      <div class="event-detail-image">
        <img src="${escapeHtml(image)}" alt="${escapeHtml(ev.title)}">
        <span class="event-date-badge">${escapeHtml(shortDate(ev.date))}</span>
      </div>
      <div class="event-detail-body">
        <span class="eyebrow">${escapeHtml(formatDate(ev.date))}</span>
