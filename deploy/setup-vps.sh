...
if [ -d "$SITE_DIR/.git" ]; then
  ...
  cp "$SITE_DIR/content/participants.json" "$CONTENT_BACKUP_DIR/" 2>/dev/null || true
  ...
  [ -f "$CONTENT_BACKUP_DIR/participants.json" ] && cp "$CONTENT_BACKUP_DIR/participants.json" "$SITE_DIR/content/participants.json"
else
